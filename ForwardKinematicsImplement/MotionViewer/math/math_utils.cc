...
{
    Vector6d_t radian6d;
    radian6d.SetAngularVector(
            math::ToRadian(degree6d.AngularVector())
            );
    radian6d.SetLinearVector(degree6d.LinearVector());
...
{
    Vector6d_t degree6d;
    degree6d.SetAngularVector(
            math::ToDegree(radian6d.AngularVector())
            );
    degree6d.SetLinearVector(radian6d.LinearVector());
...
RotMat_t ComputeRotMatX(const double radian)
{
    return AngleAxis_t(radian, Vector3d_t::UnitX()).toRotationMatrix();
}

RotMat_t ComputeRotMatY(const double radian)
{
    return AngleAxis_t(radian, Vector3d_t::UnitY()).toRotationMatrix();
}

RotMat_t ComputeRotMatZ(const double radian)
{
    return AngleAxis_t(radian, Vector3d_t::UnitZ()).toRotationMatrix();
}

RotMat_t ComputeRotMatXyz(const double radian_x, const double radian_y, const double radian_z)
{
    return ComputeRotMatZ(radian_z) * ComputeRotMatY(radian_y) * ComputeRotMatX(radian_x);
}

RotMat_t ComputeRotMatZyx(const double radian_x, const double radian_y, const double radian_z)
{
    return ComputeRotMatX(radian_x) * ComputeRotMatY(radian_y) * ComputeRotMatZ(radian_z);
}

RotMat_t ComputeRotMatXyz(const Vector3d_t &radian3d)
{
    return ComputeRotMatXyz(radian3d.x(), radian3d.y(), radian3d.z());
}

RotMat_t ComputeRotMatZyx(const Vector3d_t &radian3d)
{
    return ComputeRotMatZyx(radian3d.x(), radian3d.y(), radian3d.z());
}

Vector3d_t ComputeEulerAngleXyz(const RotMat_t &rot_mat)
{
    typedef RotMat_t::RealScalar Scalar_t;
...
Vector3d_t ComputeEulerAngleZyx(const RotMat_t &rot_mat)
{
    typedef RotMat_t::RealScalar Scalar_t;
...
RotMat_t ToRotMat(const double mat4[4][4])
{
    RotMat_t output_rotmat;
...
void ToMatrix4(const RotMat_t &rot_mat, double mat4[4][4])
{
...
Quaternion_t ToQuaternionXyz(const double radian_x, const double radian_y, const double radian_z)
{
    RotMat_t rotmat_xyz = ComputeRotMatXyz(radian_x, radian_y, radian_z);
    return Quaternion_t(rotmat_xyz);
}

Quaternion_t ToQuaternionZyx(const double radian_x, const double radian_y, const double radian_z)
{
    RotMat_t rotmat_zyx = ComputeRotMatZyx(radian_x, radian_y, radian_z);
...
RotMat_t ComputeRelativeRotMat(
