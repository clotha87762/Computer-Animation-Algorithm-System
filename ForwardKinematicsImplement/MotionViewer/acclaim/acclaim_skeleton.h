...
public: 
     ...
     */
    double BoneLength(const int32_t bone_idx) const;
    /**
     * \brief
     */
    std::string BoneName(const int32_t bone_idx) const;
    /**
     * \brief
     */
    int32_t BoneIdx(const std::string &name) const;
    /**
     * \brief
     */
    Vector3d_t BoneLocalDir(const int32_t bone_idx) const;
    /**
     ...
     * \return
     */
    const Bone *BonePtr(const int32_t bone_idx) const;

private:

    //typedef boost::ptr_vector<Bone> BoneColl_t;
    ...
    Vector3d_t root_pos_; ///< Root position in world coordinate system
