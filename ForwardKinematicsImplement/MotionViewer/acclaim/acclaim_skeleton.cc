    ...
    bone_count_(0)
{
    ...
    bone_count_ = 1;
    //Initializaton
    bone_list_[Skeleton::RootIdx()].idx = RootIdx();   // root of hierarchy
    root_bone_ = &bone_list_[Skeleton::RootIdx()];
    bone_list_[Skeleton::RootIdx()].sibling = nullptr;
    bone_list_[Skeleton::RootIdx()].child = nullptr;
    bone_list_[Skeleton::RootIdx()].parent = nullptr;
    bone_list_[Skeleton::RootIdx()].dir[0] = 0.;
    bone_list_[Skeleton::RootIdx()].dir[1] = 0.;
    bone_list_[Skeleton::RootIdx()].dir[2] = 0.;
    bone_list_[Skeleton::RootIdx()].axis.x() = 0.;
    bone_list_[Skeleton::RootIdx()].axis.y() = 0.;
    bone_list_[Skeleton::RootIdx()].axis.z() = 0.;
    bone_list_[Skeleton::RootIdx()].length = 0.05;
    bone_list_[Skeleton::RootIdx()].dof = 6;
    bone_list_[Skeleton::RootIdx()].dofx = 1;
    bone_list_[Skeleton::RootIdx()].dofy = 1;
    bone_list_[Skeleton::RootIdx()].dofz = 1;
    bone_list_[Skeleton::RootIdx()].doftx = 1;
    bone_list_[Skeleton::RootIdx()].dofty = 1;
    bone_list_[Skeleton::RootIdx()].doftz = 1;
...
Vector3d_t Skeleton::root_pos() const
{
    return root_pos_;
}
...
double Skeleton::BoneLength(const int32_t bone_idx) const
{
    return bone_list_[bone_idx].length;
}

std::string Skeleton::BoneName(const int32_t bone_idx) const
{
    int32_t i = 0;
...
int32_t Skeleton::BoneIdx(const std::string &name) const
{
...
Vector3d_t Skeleton::BoneLocalDir(const int32_t bone_idx) const
{
...
void Skeleton::SetBasePosture()
{
    ...
    {
        bone_list_[i].dr = Vector3d_t::Zero();
    }
}

const Bone *Skeleton::BonePtr(const int32_t bone_idx) const
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            while (nullptr != part_name)
            {
                if (j==0)
                {
                    parent = this->BoneIdx(part_name);
                }
                else
                {
                    this->SetChildrenAndSibling(parent, &bone_list_[this->BoneIdx(part_name)]);
                }
...
void Skeleton::RotateBoneDirToLocalCoordSystem()
{
    for (int32_t i = 1; i < this->bone_num(); i++)
    {
        ...
        Vector3d_t bone_global_dir = bone_list_[i].dir;
        RotMat_t globalToLocal_mat = math::ComputeRotMatZyx(
                math::ToRadian(-bone_list_[i].axis.x()),
                math::ToRadian(-bone_list_[i].axis.y()),
                math::ToRadian(-bone_list_[i].axis.z())
                );
        Vector3d_t bone_local_dir = globalToLocal_mat * bone_global_dir;
...
{
    bone[Skeleton::RootIdx()].aspx=1;
    bone[Skeleton::RootIdx()].aspy=1;
...
{
    RotMat_t parentCoord_trans_mat, childCoord_mat, childToParent_mat, childToParent_trans_mat;
...
{
    RotMat_t rootCoord_mat, rootCoord_trans_mat;

    rootCoord_mat = math::ComputeRotMatXyz(
            math::ToRadian(bone[Skeleton::RootIdx()].axis.x()),
            math::ToRadian(bone[Skeleton::RootIdx()].axis.y()),
            math::ToRadian(bone[Skeleton::RootIdx()].axis.z())
            );
    rootCoord_trans_mat = rootCoord_mat.transpose();
    math::ToMatrix4(
            rootCoord_trans_mat,
            bone[Skeleton::RootIdx()].rot_parent_current
            );
